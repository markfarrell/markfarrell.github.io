...
**[** assuming the fact that a vector $$v \in \mathbb{R}^{n}$$ is an eigenvector for eigenvalue $$k$$ of a $$k$$-regular graph on $$n$$ vertices if and only if $$v_j = c$$ for all $$ 1 \leq j \leq n$$ and some non-zero constant $$c \in \mathbb{R}$$, and letting $$J$$ denote the $$n \times n$$ square matrix consisting of all ones **]**. This means, in particular, that:
