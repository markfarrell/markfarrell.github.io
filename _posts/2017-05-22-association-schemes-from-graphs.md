...
and $$\mathcal{A} := \{ A_{n} : 1 \leq n \leq \operatorname{diam}(G) \}$$ denote a set of distance matrices for $$G$$.
Additionally, define $$S_{i,j}(u,v) := \{ w : w \in \mathcal{V}_{G} \operatorname{and} (d(u, w), d(w, v)) = (i, j) \}$$ for all 
$$1 \leq i,j \leq \operatorname{diam}(G)$$ and all $$u,v \in \mathcal{V}_{G}$$. Now, select any particular $$1 \leq i,j \leq \operatorname{diam}(G)$$. Then for any $$w, x \in \mathcal{V}_{G}$$:
...
$$ \implies A_{i} A_{j} = A_{j} A_{i} = \sum\limits_{k = 1}^{\operatorname{diam}(G)} p_{i,j}(k) \ A_{k}  \text{ for some } \{ p_{i,j}(k) : 1 \leq k \leq \operatorname{diam}(G) \} \subset \mathbb{N} $$

$$ \implies \mathcal{A} \text{ is a commutative association scheme } $$

**[** since $$i$$ and $$j$$ are arbitrary, in addition to the facts that $$A_{0} = I \in \mathcal{A}$$, $$\sum\limits_{k = 1}^{\operatorname{diam}(G)} A_k = J$$, and $$A_{k}^{T} = A_{k}$$ for all $$1 \leq k \leq \operatorname{diam}(G)$$ by construction **]**.
...
Next, observe that: 
 ...
 $$ \implies \mathcal{A} \text{ is a commutative assocation scheme } $$
